...
if(isset($_GET['action']) && $_GET['action']==22){
	...
	if($resposta==1){
		$grava = mysql_query("INSERT INTO foto_bercario (id_bercario, foto) VALUES ('$id', '$nm_new')");	
	...
	$exclui = mysql_query("DELETE FROM foto_bercario WHERE id='$id_foto' LIMIT 1");
...
$pega = mysql_query("SELECT nome_crianca FROM bercario WHERE id='$id' LIMIT 1");
...
<tr>
<td align="left"><span class="titu">:: Enviando<b> fotos </b>para o bêbe: <b><?php echo(strip_tags($row['nome_crianca'])); ?></b></span></td>
</tr>
...
<form method="post" action="fotos_bercario.php?action=22" enctype="multipart/form-data">	
<table cellpadding="0" cellspacing="0" width="100%" >
...
<tr><td align="right"><span class="titu">Fotos cadastradas para o bêbe <b><?php echo(strip_tags($row['nome_crianca'])); ?></b> ::</span></td></tr>
<tr><Td height="4"></Td></tr>
...
$pega2 = mysql_query("SELECT * FROM foto_bercario WHERE id_bercario='$id' ORDER BY id DESC");
			...
			if(mysql_num_rows($pega2)){
				...
				while($row2 = mysql_fetch_array($pega2)){
					...
					if($i==7){$i=0; echo '</tr><tr><td height="16"></td></tr><tr>';}
						...
						<img src="../thumbs.php?img=imagens/'.$row2['foto'].'&altura=70&largura=100" style="border:1px solid black"><br>
